...
class ClusterHealth(Runner):
    ...
    async def __call__(self, opensearch, params):
        ...
        print("Executing Cluster Health!!!!!!!")
        try:
            api_kw_params = self._default_kw_params(params)
            some_result = await opensearch.cluster.health(**api_kw_params)
        finally:
            print('>>>>>>>>>>>async executed successfully')
        return {
            "weight": 1,
            "unit": "ops",
            "success": True,
            "cluster-status": status('green'),
            "relocating-shards": 0
        }
    ...
    async def __call__(self, opensearch, params):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                print(">>>>>>>>>> Transport exeception found!!!!")
                print(e)
                print(e.status_code)
                if last_attempt or not retry_on_timeout:
                    raise e
