...
from opensearch_benchmark import config, exceptions, telemetry
from opensearch_benchmark.builder import launcher, cluster
from opensearch_benchmark.builder.provisioner import NodeConfiguration
from opensearch_benchmark.metrics import InMemoryMetricsStore
...
class ProcessLauncherTests(TestCase):
    ...
    @mock.patch('opensearch_benchmark.builder.java_resolver.java_home', return_value=(12, "/java_home/"))
    @mock.patch('opensearch_benchmark.utils.jvm.supports_option', return_value=True)
    @mock.patch('opensearch_benchmark.utils.io.get_size')
    @mock.patch('os.chdir')
    @mock.patch('opensearch_benchmark.builder.launcher.wait_for_pidfile', return_value=MOCK_PID_VALUE)
...
class DockerLauncherTests(TestCase):
    ...
    @mock.patch("opensearch_benchmark.utils.process.run_subprocess_with_output")
    ...
    @mock.patch("opensearch_benchmark.telemetry.add_metadata_for_node")
    @mock.patch("opensearch_benchmark.utils.process.run_subprocess_with_logging")
